...
class Bnode
{
	...
	Bnode(int d)
	{
		//Default Constructor setting For default value for height ,and pointers 
		data = d;
		height = 0;
		left = right = NULL;
	}
...
class AVL_Trees
{
	public:
		...
		{
			//getheight function for calculating height of the binarysearch tree
			if(root==NULL)
			{
				return -1;
			}
		...
		Bnode* insert(Bnode* root,int data)
		{
			...
			if(root->data > data)
			{
				...
				int h = getHeight(root->left) - getHeight(root->right);    //BALANCE FACTOR 
				if(h==2 || h==-2)
				{
			...
			//	root->right = insert(root->right,data);
				...
				if(h==2 || h==-2)
				{
					...
					{
					//		cout<<"Right Left rotation of "<<root->val<<" With "<<root->left->val<<endl;
		...
		switch(choice)
		{
			...
			case 4:
				if(root==NULL)
					...
					cout<<"\nThe Height of Avl Tree is -> "<<(o.getHeight(root)+1)<<endl;
