...
problem statement link -> https://leetcode.com/submissions/detail/543396619/
...
Capitalization: If the query matches a word in the wordlist (case-insensitive), then the query word is returned with the same 
...
public:
    vector<string> spellchecker(vector<string>& wordlist, vector<string>& queries) 
	{
      ...
      for(string i:queries)
      {
        ...
        if(capital.find(temp)!=capital.end())
        {
          answer.push_back(capital[temp]);
          continue;
        }
         if(vowel.find(temp2)!=vowel.end())
        {
            answer.push_back(vowel[temp2]);
        }
        else
        {
          answer.push_back(""); 
