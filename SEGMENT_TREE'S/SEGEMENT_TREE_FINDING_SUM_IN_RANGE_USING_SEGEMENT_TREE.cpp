...
int builttree(int left,int right,int x,int arr[],int array[])
{
	if(left<=right)
	{
		...
		int mid = (left+right)/2;
		 array[x]  = builttree(left,mid,(x*2)+1,arr,array)+builttree(mid+1,right,(x*2)+2,arr,array);
		 return array[x];
	}
}
int getans(int left,int right,int ql,int qr,int x,int array[])
{
	if(left<=right)
	{
		...
		int mid = (left+right)/2;
		return getans(left,mid,ql,qr,(x*2)+1,array)+getans(mid+1,right,ql,qr,(x*2)+2,array);
	}
}
